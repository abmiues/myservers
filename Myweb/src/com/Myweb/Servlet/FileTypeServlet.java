...
public class FileTypeServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
				...
				if(lb==null){
					rd=request.getRequestDispatcher("/TypeServlet");
					
					rd.forward(request, response);
					}
				else{
				
				int lbid=Integer.parseInt(lb);
				

				FileService file=new FileService();
				List<FileType> fileTypeList=file.getALLtype();
				request.setAttribute("fileTypeList", fileTypeList);
				
				List<UpFile> filelist=file.getFileByLb(lbid);
